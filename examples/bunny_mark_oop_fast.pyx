...
cdef class Game(App):
    ...
    def init(self):
        ...
        self.texture.create(format=TEXTURE_FORMAT_RGBA_8U, mipmaps=True, filter=TEXTURE_FILTER_LINEAR)
    ...
    def on_enter_frame(self, event_data, *args, **kwargs):
        ...
        for i in range(self.num_sprites):
            Vec2.c_random(&shift)
            Vec2.c_scale_add(&shift, &shift, 2.0, -1.0)
            sprite = self.sprite_batch.get_ptr().sprites[i]
