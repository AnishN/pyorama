...
cdef inline Error vector_init(VectorC *vector, size_t item_size) nogil:
...
cdef inline void vector_free(VectorC *vector) nogil:
    vector.max_items = 0
    vector.item_size = 0
    vector.num_items = 0
    free(vector.items)
...
cdef inline Error vector_push_empty(VectorC *vector) nogil:
    cdef:
        uint8_t *item
        Error error
...
cdef inline Error vector_pop_empty(VectorC *vector) nogil:
    cdef:
        Error error
...
cdef inline Error vector_push(VectorC *vector, void *item) nogil:
    cdef:
        Error error
...
cdef inline Error vector_pop(VectorC *vector, void *item) nogil:
    if vector.num_items <= 0:
        return POP_EMPTY_ERROR
    error = vector_shrink_if_needed(vector)
    if error == MEMORY_ERROR:
...
cdef inline void *vector_get_ptr_unsafe(VectorC *vector, size_t index) nogil:
    return vector.items + (vector.item_size * index)

cdef inline Error vector_get_ptr(VectorC *vector, size_t index, void **item_ptr) nogil:
    if 0 <= index < vector.max_items: 
        item_ptr[0] = vector.items + (vector.item_size * index)
    else:
        return INVALID_INDEX_ERROR

cdef inline Error vector_get(VectorC *vector, size_t index, void *item) nogil:
    cdef:
        uint8_t *src
...
cdef inline Error vector_set(VectorC *vector, size_t index, void *item) nogil:
    cdef:
        uint8_t *dst
...
cdef inline Error vector_clear(VectorC *vector, size_t index) nogil:
    cdef:
        uint8_t *dst
...
cdef inline void vector_clear_all(VectorC *vector) nogil:
    memset(vector.items, 0, vector.max_items * vector.item_size)

cdef inline Error vector_swap(VectorC *vector, size_t a, size_t b) nogil:
    cdef:
        uint8_t *a_ptr
        uint8_t *b_ptr
        size_t i
...
cdef inline Error vector_resize(VectorC *vector, size_t new_max_items) nogil:
    cdef:
        uint8_t *new_items
        uint8_t *clear_start
        size_t clear_size
...
cdef inline Error vector_grow_if_needed(VectorC *vector) nogil:
    cdef:
...
cdef inline Error vector_shrink_if_needed(VectorC *vector) nogil:
    cdef:
        size_t new_max_items
        Error error
...
cdef inline Error vector_insert_empty(VectorC *vector, size_t index) nogil:
    cdef:
        void *dst
        void *src
        size_t size
...
cdef inline Error vector_insert(VectorC *vector, size_t index, void *item) nogil:
    cdef:
        void *dst
        void *src
        size_t size
...
cdef inline Error vector_remove_empty(VectorC *vector, size_t index) nogil:
    cdef:
        void *dst
        void *src
        size_t size
...
cdef inline Error vector_remove(VectorC *vector, size_t index, void *item) nogil:
    cdef:
        void *dst
        void *src
        size_t size
...
cdef inline Error vector_find(VectorC *vector, void *item, size_t *index) nogil:
    cdef:
        size_t i
        void *test_item
        int check
...
cdef inline bint vector_contains(VectorC *vector, void *item) nogil:
    cdef:
        size_t i
        void *test_item
        int check
