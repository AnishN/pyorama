...
cdef inline Error int_hash_map_init(IntHashMapC *hash_map) nogil:
    cdef:
        Error error
...
cdef inline void int_hash_map_free(IntHashMapC *hash_map) nogil:
    vector_free(&hash_map.items)
    hash_map.num_items = 0

cdef inline Error int_hash_map_insert(IntHashMapC *hash_map, uint64_t key, uint64_t value) nogil:
    cdef:
        Error error
        size_t i
        uint64_t hashed_key
...
cdef inline Error int_hash_map_remove(IntHashMapC *hash_map, uint64_t key) nogil:
    cdef:
        Error error
        size_t index
        ItemC *item_ptr
...
cdef inline Error int_hash_map_get(IntHashMapC *hash_map, uint64_t key, uint64_t *value) nogil:
    cdef:
        Error error
        size_t index
        ItemC *item_ptr
...
cdef inline Error int_hash_map_get_index(IntHashMapC *hash_map, uint64_t key, size_t *index_ptr) nogil:
    cdef:
        size_t i
        uint64_t hashed_key
        size_t index
...
cdef inline uint64_t int_hash_map_hash(uint64_t key) nogil:
...
cdef inline bint int_hash_map_contains(IntHashMapC *hash_map, uint64_t key) nogil:
    cdef:
        size_t i
        uint64_t hashed_key
        size_t index
...
cdef inline Error int_hash_map_grow_if_needed(IntHashMapC *hash_map) nogil:
    cdef:
...
cdef inline Error int_hash_map_shrink_if_needed(IntHashMapC *hash_map) nogil:
    cdef:
        size_t new_max_items
...
cdef inline Error int_hash_map_resize(IntHashMapC *hash_map, size_t new_max_items) nogil: 
    cdef:
        ItemC *new_items
        uint64_t hashed_key
        size_t i, j
