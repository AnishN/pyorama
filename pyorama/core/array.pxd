...
cdef inline Error array_init(ArrayC *array, size_t item_size, size_t max_items) nogil:
...
cdef inline void array_free(ArrayC *array) nogil:
    array.max_items = 0
    array.item_size = 0
    free(array.items)
    array.items = NULL

cdef inline void *array_get_ptr_unsafe(ArrayC *array, size_t index) nogil:
    return array.items + (array.item_size * index)

cdef inline Error array_get_ptr(ArrayC *array, size_t index, void **item_ptr) nogil:
    if 0 <= index < array.max_items: 
        item_ptr[0] = array.items + (array.item_size * index)
    else:
        return INVALID_INDEX_ERROR

cdef inline Error array_get(ArrayC *array, size_t index, void *item) nogil:
    cdef:
        uint8_t *src
...
cdef inline Error array_set(ArrayC *array, size_t index, void *item) nogil:
    cdef:
        uint8_t *dst
...
cdef inline Error array_clear(ArrayC *array, size_t index) nogil:
    cdef:
        uint8_t *dst
...
cdef inline void array_clear_all(ArrayC *array) nogil:
    memset(array.items, 0, array.max_items * array.item_size)

cdef inline Error array_swap(ArrayC *array, size_t a, size_t b) nogil:
    cdef:
        uint8_t *a_ptr
        uint8_t *b_ptr
        size_t i
...
cdef inline Error array_find(ArrayC *array, void *item, size_t *index) nogil:
    cdef:
        size_t i
        void *test_item
        int check
...
cdef inline bint array_contains(ArrayC *array, void *item) nogil:
    cdef:
        size_t i
        void *test_item
        int check
