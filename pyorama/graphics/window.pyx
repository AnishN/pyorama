...
cdef class Window:
    ...
    cpdef void render(self) except *:
        ...
        if handle_get_type(&window_ptr.texture) == Texture.c_get_type():
            ...
            self.manager.quad_program.c_bind_uniform(self.manager.u_quad.handle)
            self.manager.quad_program.c_bind_attributes(self.manager.quad_vbo.handle)
            self.manager.quad_ibo.c_draw()
            self.manager.quad_program.c_unbind_attributes()
