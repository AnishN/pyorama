...
cdef void item_vector_push_empty(ItemVectorC *self) except *:
    ...
    if self.num_items >= self.max_items:
        ...
        item_vector_resize(self, new_max_items)
...
cdef void item_vector_push(ItemVectorC *self, void *item) except *:
    ...
    if self.num_items >= self.max_items:
        new_max_items = <size_t>(self.max_items * VECTOR_GROWTH_RATE)
        item_vector_resize(self, new_max_items)
    item_vector_set(self, self.num_items, item)
    self.num_items += 1
...
cdef void item_vector_pop(ItemVectorC *self, void *item) except *:
    ...
    if self.num_items <= 0:
        ...
        new_max_items = <size_t>(self.max_items * VECTOR_SHRINK_RATE)
        item_vector_resize(self, new_max_items)
    item_vector_get(self, self.num_items - 1, item)
    self.num_items -= 1
...
cdef void item_vector_swap(ItemVectorC *self, size_t a, size_t b) nogil:
    ...
    if (0 <= a < self.num_items) and (0 <= b < self.num_items):
        item_vector_get_ptr(self, a, <void **>&a_ptr)
        item_vector_get_ptr(self, b, <void **>&b_ptr)
