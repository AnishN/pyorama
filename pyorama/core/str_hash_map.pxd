...
cdef inline bint item_match(StrHashMapItemC *item_ptr, char *key, size_t key_len, uint64_t hashed_key) nogil:
    if item_ptr.hashed_key == hashed_key and item_ptr.key_len == key_len:
        if memcmp(item_ptr.key, key, key_len) == 0:
            return True
    return False

cdef inline void item_set(StrHashMapItemC *item_ptr, char *key, size_t key_len, uint64_t hashed_key, uint64_t value, bint used) nogil:
    item_ptr.key = key
...
cdef inline Error str_hash_map_init(StrHashMapC *hash_map) nogil:
    cdef:
        Error error
...
cdef inline void str_hash_map_free(StrHashMapC *hash_map) nogil:
    vector_free(&hash_map.items)
    hash_map.num_items = 0

cdef inline void str_hash_map_insert(StrHashMapC *hash_map, char *key, size_t key_len, uint64_t value) nogil:
    cdef:
        size_t i
        uint64_t hashed_key
        size_t index
...
cdef inline void str_hash_map_remove(StrHashMapC *hash_map, char *key, size_t key_len) nogil:
    cdef:
        size_t index
        StrHashMapItemC *item_ptr
...
cdef inline uint64_t str_hash_map_get(StrHashMapC *hash_map, char *key, size_t key_len) nogil:
    cdef:
        size_t index
        StrHashMapItemC *item_ptr
...
cdef inline Error str_hash_map_get_index(StrHashMapC *hash_map, char *key, size_t key_len, size_t *index_ptr) nogil:
    cdef:
        size_t i
        uint64_t hashed_key
        size_t index
...
cdef inline uint64_t str_hash_map_hash(char *key, size_t key_len) nogil:
    return XXH3_64bits(key, key_len)

cdef inline bint str_hash_map_contains(StrHashMapC *hash_map, char *key, size_t key_len) nogil:
    cdef:
        size_t i
        uint64_t hashed_key
        size_t index
...
cdef inline void str_hash_map_grow_if_needed(StrHashMapC *hash_map) nogil:
...
cdef inline void str_hash_map_shrink_if_needed(StrHashMapC *hash_map) nogil:
    cdef size_t new_max_items
...
cdef inline Error str_hash_map_resize(StrHashMapC *hash_map, size_t new_max_items) nogil: 
    cdef:
...
cdef inline Error str_hash_map_extend(StrHashMapC *hash_map_a, StrHashMapC *hash_map_b, bint overwrite=True) nogil:
    cdef:
        size_t i, j
        size_t index
